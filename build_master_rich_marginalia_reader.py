...
for wid, cdata in CHAPTERS.items():
    for term in cdata.get("terms", []):
        ...
        if key not in glossary_dict:
            glossary_dict[key] = {
                "name": term,
                "definition": f"Operational dynamic formulated in World {wid:02d}: {cdata['title']}.",
                "world_id": wid,
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* Genuine Tactile Paper Grain + Vignette */
  body::after {
  ...
  body.mode-crisp .monograph-plate-image {
    mix-blend-mode:normal;
    -webkit-mask-image:none;
    mask-image:none;
    filter:contrast(116%) brightness(98%) grayscale(100%) sepia(18%);
  }
  ...
  #side-marginalia-drawer {
    position:fixed; top:54px; right:-460px; width:440px; height:calc(100vh - 54px);
...
<div id="reader-overlay-view">
  <div class="reader-header-bar">
    ...
    <div class="mode-switch-bar">
      ...
      <button class="mode-chip active" id="btn-mode-paper" onclick="setImageDisplayMode('paper')">PULP BLEND</button>
      <button class="mode-chip" id="btn-mode-crisp" onclick="setImageDisplayMode('crisp')">CRISP ARCHIVE</button>
...
<div id="side-marginalia-drawer">
  ...
  <div style="font-family:var(--serif); font-size:1.05rem; line-height:1.65; color:var(--ink); margin-bottom:16px; background:var(--paper); padding:12px 14px; border-left:3px solid var(--red);" id="drawer-term-desc">
    Definition.
  </div>
  ...
  <div class="spec-section-card">
    <div class="spec-section-head">
      ...
      Philosophical Lineage & Thinkers
    </div>
  ...
  <div class="spec-section-card">
    <div class="spec-section-head">
      ...
      Material Ancestry & Field Practices
    </div>
  ...
  <div class="spec-section-card">
    <div class="spec-section-head">
      ...
      Computational Logic & State Model
    </div>
    <div style="font-family:var(--mono); font-size:.78rem; line-height:1.5; color:var(--ink); background:var(--paper-hi); padding:8px 10px; border-left:2px solid var(--red); white-space:pre-wrap;" id="drawer-computational-info">—</div>
  </div>
  ...
  <div class="spec-section-card">
    <div class="spec-section-head">
      ...
      Formal Theory Skeleton & Transitions
    </div>
    <div style="font-family:var(--mono); font-size:.74rem; line-height:1.5; color:var(--ink); margin-bottom:8px;" id="drawer-skeleton-info">—</div>
    <div style="font-family:var(--mono); font-size:.74rem; line-height:1.5; color:var(--red); font-weight:700;" id="drawer-operational-info">—</div>
  </div>
  ...
  <div class="spec-section-card">
    ...
    <div style="font-family:var(--serif); font-size:.88rem; line-height:1.55; color:var(--ink); font-style:italic;" id="drawer-changetest-info">—</div>
  </div>
</div>
...
<script>
  ...
  /* ============ RENDER NATIVE MONOGRAPH STREAM WITH RICH HYPERLINKS ============ */
      ...
      return `
        <article class="monograph-world-block" id="monograph-block-${ch.id}" data-id="${ch.id}">
          ...
          <div style="margin-top:24px; display:flex; flex-wrap:wrap; gap:8px; align-items:center;">
            ...
            ${ch.terms.map(t => `
              <button onclick="openConceptSideDrawer('${t.replace(/'/g, "\\\\'")}', ${ch.id})" style="background:var(--paper-hi); border:1px solid var(--hair); padding:4px 9px; font-family:var(--mono); font-size:.72rem; color:var(--red); cursor:pointer; font-weight:600;">
                ${t} &rarr;
              </button>
            `).join('')}
          </div>
  ...
  function openConceptSideDrawer(rawTerm, wid) {
    ...
    document.getElementById('drawer-term-desc').innerText = item ? item.definition : (ch ? ch.invariant : "Operational field invariant under active investigation.");
    ...
    document.getElementById('drawer-computational-info').innerText = ch ? (ch.computational || "# State machine active") : "#";
    document.getElementById('drawer-skeleton-info').innerText = ch ? (ch.skeleton || "Entities & Operations") : "";
    document.getElementById('drawer-operational-info').innerText = ch ? (ch.operational || "Operational Trace") : "";
...
print(f"Generated Rich Marginalia Master WORLDFUL Press Application ({len(html_rendered):,} bytes)")
